...
describe('transaction reducer', () => {
  ...
  describe('addTransaction', () => {
    it('adds the transaction', () => {
      ...
      expect(txs[ChainId.BSCTESTNET]).toBeTruthy()
      expect(txs[ChainId.BSCTESTNET]?.['0x0']).toBeTruthy()
      const tx = txs[ChainId.BSCTESTNET]?.['0x0']
  ...
  describe('clearAllTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.BSCTESTNET), String(ChainId.RINKEBY)])
      expect(Object.keys(store.getState()[ChainId.BSCTESTNET] ?? {})).toEqual(['0x0'])
      expect(Object.keys(store.getState()[ChainId.RINKEBY] ?? {})).toEqual(['0x1'])
      store.dispatch(clearAllTransactions({ chainId: ChainId.BSCTESTNET }))
      expect(Object.keys(store.getState())).toHaveLength(2)
      expect(Object.keys(store.getState())).toEqual([String(ChainId.BSCTESTNET), String(ChainId.RINKEBY)])
      expect(Object.keys(store.getState()[ChainId.BSCTESTNET] ?? {})).toEqual([])
      expect(Object.keys(store.getState()[ChainId.RINKEBY] ?? {})).toEqual(['0x1'])
    })
  })
})
